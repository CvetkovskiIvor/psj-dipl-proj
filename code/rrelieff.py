...
x = StandardScaler().fit_transform(pd.read_csv('../data/intermediate_data/unique_descriptors.csv', index_col='ID').values)
y = pd.read_csv('../data/y.csv', index_col='ID').values

method = RReliefF()
method.fit(x, y)

data = dict(Descriptor=list(pd.read_csv('../data/intermediate_data/unique_descriptors.csv', index_col='ID').columns),
